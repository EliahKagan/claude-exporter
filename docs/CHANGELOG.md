...
- The browse page no longer waits forever for a claude.ai tab that cannot answer. A tab is chosen only if it is loaded and not discarded by Chrome's memory saver, the page looks in the current window before searching the others, and the relay gives up after 10 seconds instead of showing a spinner indefinitely. Previously it took the first claude.ai tab in any window and, if that tab's content script held the message channel open without replying, nothing timed out and nothing was reported.
